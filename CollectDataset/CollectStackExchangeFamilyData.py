...
def gen_url(section):
    ...
    urls = []
    urls.append('https://archive.org/download/stackexchange/' + section + '.meta.stackexchange.com.7z')
...
def un7zip(path, folder):
    outfilename = os.path.join(path)
    archive = py7zr.SevenZipFile(outfilename, mode='r')
    pwd = os.getcwd()
    os.chdir(folder)
    archive.extractall()
    os.chdir(pwd)
...
def load(url, file_name, folder):
    ...
    if not os.path.isfile(file_name):
        ...
        try:
            ...
            try:
                r = request.get(url[1], allow_redirects=True)
                open(file_name, 'wb').write(r.content)
            except:
